...
export default function AttendanceAdminPage() {
  ...
  // Initialize camera scanner when mode === 'camera' with facingMode (environment / rear by default)
  useEffect(() => {
    if (mode !== 'camera') {
      stopCamera();
      return;
    ...
    async function initScanner() {
      ...
      try {
        ...
        console.warn('Primary facingMode camera failed, trying fallback camera:', err);
        try {
          if (html5QrCodeRef.current) {
          ...
          <div className="glass-panel p-6 rounded-3xl border border-zinc-800 space-y-4 text-center">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-b border-zinc-800 pb-3">
              ...
              <button
                ...
                className="px-3.5 py-2 bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 transition shrink-0"
              >
                <RefreshCw className="w-3.5 h-3.5 text-red-500" />
                <span>{facingMode === 'environment' ? '📷 Arka Kamera (Aktif)' : '🤳 Ön Kamera (Aktif)'}</span>
              </button>
            </div>
            ...
            <p className="text-[11px] text-zinc-500">
              Cihaz kameranıza fiziksel yaka kartını yaklaştırdığınızda yoklama otomatik kaydedilir. İhtiyaç halinde yukarıdaki butondan kamerayı değiştirebilirsiniz.
            </p>
          </div>
